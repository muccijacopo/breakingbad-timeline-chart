<template>
    <g class="character-entry">
        ...
        <g :class="'character-' + character.name" @mouseover="onMouseHover($event)" @mouseleave="onMouseLeave($event)">
...
function onMouseHover(event: MouseEvent) {
    emit('onCharacterHover', { characterId: props.character.id, x: event.x, y: event.y })
}
